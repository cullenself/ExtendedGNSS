...
for i = 1:length(visible)
    visible(i).power = SatPowerOut(visible(i).SVID) + SatDirectivityGain(visible(i).SVID,visible(i).offboreTX) + SatGainCF(visible(i).SVID) + 20*log10(lambdaGPSL1/(4*pi*norm(visible(i).rel))) + RXGain(visible(i).offboreRX); % multiple things need to be checked out, log vs log10, need to make SatPowerOut matrix, SatDGain depends on another angle that I don't know, must make up RXGain
